...
// Interpolation routine to get MAC grid from velocity center (uses GetMAC
// kernel from source/commonkernels.h).
PYTHON() void getMACFromVec3(MACGrid& target, Grid<Vec3>& source) {
  GetMAC(target, source);
}

// Interpolation routine to get Vel3 grid center from MAC grid (uses GetCentered
// kernel from source/commonkernels.h).
PYTHON() void getVec3FromMAC(Grid<Vec3>& target, MACGrid& source) {
  GetCentered(target, source);
}
